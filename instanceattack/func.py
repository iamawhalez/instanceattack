  ...
  def update_returns(self, set_default=False):
    for ins in filter(lambda x: x.f_exit, self.instrs):
      if set_default:
        ...
        ins.USE.update(set(('eax',)) | self.pre_regs)#REGS[:8])
        ins.implicit.update(set(('eax',)) | self.pre_regs)#REGS[:8])
      else:
        ins.USE.update(self.ret_regs | self.pre_regs)
        ins.implicit.update(self.ret_regs | self.pre_regs)
